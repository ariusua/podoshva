            ...
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    if (data.messages == 'ok') {
                        ...
                        $('#' + prefix + 'comment_work_' + comment_id).append('<div class="warning alert alert-danger">' + data.success + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                        remove_success();
                    }
                }
            }
...
function comment_write(event) {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            ...
            $('#' + prefix + 'comment_work_' + myid).before('<div class="attention alert alert-attention">'+text_wait+'<img src="catalog/view/theme/' + theme + '/image/aloading16.png" class="aloading" alt="">' +  '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
            wisybbdestroy(prefix, acr);
        },
        error: function () {
            $('.cmswidget .success, .cmswidget .warning, .cmswidget .attention, .cmswidget  .alert').remove();
        ...
        success: function (data) {
            ...
            if (wdata['code'] == 'error') {
            	$('#' + prefix + 'comment_work_' + myid).show();
           		// ***XXX*** $('#' + prefix + 'comment_work_' + myid).prepend('<div class="warning alert alert-danger">' + wdata['message'] + ' <img src="catalog/view/theme/default/image/aclose.png" alt="" class="close"><button type="button" class="close" data-dismiss="alert">&times;</button></div>');
				$('#' + prefix + 'comment_work_' + myid).prepend('<div class="warning alert alert-danger">' + wdata['message'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
            }

            if (wdata['code'] == 'success') {
                $.when($('#' + prefix + 'comment_' + mark_id).comments(acr, acc)).done(function () {
	                // ***XXX*** $('#' + prefix + 'comment_work_' + myid).prepend('<div class="success alert alert-success">' + wdata['message'] + ' <img src="catalog/view/theme/default/image/aclose.png" alt="" class="close"><button type="button" class="close" data-dismiss="alert">&times;</button></div>');
					$('#' + prefix + 'comment_work_' + myid).prepend('<div class="success alert alert-success">' + wdata['message'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                    remove_success();
                });
